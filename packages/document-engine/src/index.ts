...
const generateQRImage = async (verificationCode: string, documentHash?: string): Promise<Buffer> => {
  const verificationBaseUrl =
    process.env.MEGAZEN_DOCUMENT_VERIFY_URL ??
    "http://localhost:3000/verify/bl";
  ...
  const qrBase64 = await QRCode.toDataURL(
    documentHash ? `${verificationUrl}|BL_HASH=${documentHash}` : verificationUrl,
...
export async function renderBillOfLading(
  ...
  const qrImage = await generateQRImage(document.verificationCode, contentHash);
