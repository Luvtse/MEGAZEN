...
import type { BookingReference } from "@/lib/api/bookings";

export default function BookingDetailPage({ params }: { params: { id: string } }) {
  const [booking, setBooking] = useState<BookingReference | null>(null);
  const [error, setError] = useState<string | null>(null);
  useEffect(() => {
    // The detail URL uses the database id, but the business identity remains the 10-digit number.
    void fetch(`${process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:4000"}/api/bookings/${encodeURIComponent(params.id)}`, {
      headers: { "x-tenant-id": process.env.NEXT_PUBLIC_TENANT_ID ?? "MEGAZEN-DEMO" }, cache: "no-store",
    }).then(async r => { const p = await r.json() as {success:boolean;data:BookingReference;error?:{message?:string}}; if (!r.ok || !p.success) throw new Error(p.error?.message ?? "Booking not found"); return p.data; }).then(setBooking).catch((e: unknown)=>setError(e instanceof Error?e.message:"Booking not found"));
  }, [params.id]);
