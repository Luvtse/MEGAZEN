...
export function BillOfLadingContainers({
  ...
  const headers = {
    ...
    "x-tenant-id": process.env.NEXT_PUBLIC_TENANT_ID ?? "MEGAZEN-DEMO"
  };

  const load = async (): Promise<void> => {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:4000"}/api/bills-of-lading/${billOfLadingId}/containers`,
      {
        headers
      }
    );
  ...
  const loadContainers = async (): Promise<void> => {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:4000"}/api/containers?limit=100`,
      {
        headers
      }
    );
  ...
  const remove = async (containerId: string): Promise<void> => {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:4000"}/api/bills-of-lading/${billOfLadingId}/containers/${containerId}`,
      {
        method: "DELETE",
        headers
      }
